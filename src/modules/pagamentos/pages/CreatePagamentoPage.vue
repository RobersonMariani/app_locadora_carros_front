...
async function handleSubmit() {
  const parsed = createPagamentoSchema.safeParse(data.value)
  if (!parsed.success) {
  ...
  await submit(async (formData) => {
    ...
    uiStore.notify('success', 'Pagamento criado com sucesso.')
...
<template>
  <div>
    <nav class="mb-6 flex items-center gap-2 text-sm text-surface-500">
      ...
      <span class="font-semibold text-surface-900">Novo Pagamento</span>
    </nav>
